/**
 * @file admin.js
 * @description Client-side logic for the ABBA-360 .env Variables Editor. Handles auto-expanding inputs, variable block movement, collapsing, and saving state.
 */
...
/**
 * @function render
 * @description Flushes the container and iterates over the `envItems` array to draw the UI. Respects the 'collapsed' state of sections to hide/show their child variables.
 * @returns {void}
 */
function render() {
        ...
        if (item.type === 'section') {
            ...
            htmlBuffer += `
                <div class="env-item var-item" id="item-${index}" style="${displayStyle}">
                    ...
                    <div class="row">
                        <input type="text" class="key-input" id="key-${index}" value="${escapeHTML(item.key)}" title="Edit Variable Key">
...
/**
 * @function removeItem
 * @description Syncs the DOM state, destroys a specific block, and re-renders the UI. Warns if removing a parent section.
