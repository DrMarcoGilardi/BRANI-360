...
export class MarzipanoViewerProvider extends BaseViewerProvider {
    ...
    async init() {
        ...
        // --- THE STATE-AWARE HOOKS ---
        const sceneEl = document.querySelector('a-scene');
        if (sceneEl) {
            sceneEl.addEventListener('enter-vr', () => {
                this.inVR = true;
                const overlay = document.getElementById('tour-graph-overlay');

                const rect = this.container.getBoundingClientRect();
                const w = rect.width > 0 ? rect.width : (window.innerWidth || 800);
                const h = rect.height > 0 ? rect.height : (window.innerHeight || 600);
                ...
                if (overlay) {
                    ...
                    // Force the background to opaque #141414 for the VR renderer
                ...
                // ACTIVATE RADIAL MASK: Snapshots perfectly in html2canvas to hide square corners
                document.querySelectorAll('.vr-node-mask').forEach(el => {
                    el.style.background = 'radial-gradient(circle closest-side, transparent 88%, #141414 93%)';
                });
            });
            ...
            sceneEl.addEventListener('exit-vr', () => {
                ...
                // DEACTIVATE RADIAL MASK: Restores pristine 2D transparent UI
                document.querySelectorAll('.vr-node-mask').forEach(el => {
    ...
    setupUI(scenes) {
        ...
        const nodeMap = {};
        const sceneEl = document.querySelector('a-scene');
        const isCurrentlyVR = sceneEl && sceneEl.is('vr-mode');

        scenes.forEach(scene => {
            // Guaranteed to exist in all Marzipano exports
            const previewUrl = `${this.tourPath}/tiles/${scene.id}/preview.jpg`;
            ...
            Object.assign(nodeElement.style, {
                ...
                borderRadius: '50%', // Native 2D clipping
                overflow: 'hidden',  // Native 2D clipping
            ...
            // 1. The Image (Stretched to isolate top square)
            ...
            // 2. The VR Corner Mask (Radial Gradient)
            const maskElement = document.createElement('div');
            maskElement.className = 'vr-node-mask';
            Object.assign(maskElement.style, {
                position: 'absolute', top: '0', left: '0',
                width: '100%', height: '100%',
                pointerEvents: 'none',
                background: isCurrentlyVR
                    ? 'radial-gradient(circle closest-side, transparent 88%, #141414 93%)'
                    : 'transparent'
            });

            // 3. The Cyan Ring (Sitting on top of the seam)
            ...
            nodeElement.appendChild(maskElement);
            nodeElement.appendChild(ringElement);

            nodeElement.onmouseenter = () => ringElement.style.borderColor = '#ffffff';
            nodeElement.onmouseleave = () => ringElement.style.borderColor = '#00f0ff';
